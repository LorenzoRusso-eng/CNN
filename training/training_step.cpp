...
float train_batch_chunk(
    ...
    cuda_backend::CudaBatchTensor<float> &target_batch, const Dataset4D &input, const Dataset4D &output,
...
float train_batch_chunk_nesterov(
    ...
    const Dataset4D &input, const Dataset4D &output, const std::vector<int> &indices, int start, int end,
    const Loss &loss, const Activation &hidden_activation, const Activation &output_activation,
