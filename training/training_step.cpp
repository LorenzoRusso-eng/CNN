...
float train_batch_chunk(
    ...
    forwardprop_batch(architecture, runtime, cuda_params, hidden_activation, output_activation);

    const float loss_value = Loss_fun(runtime[num_layers - 1], loss, target_batch);
    backprop_batch(architecture, runtime, batch_gradients, cuda_params, loss, target_batch, hidden_activation, output_activation);
    return loss_value;
}
...
float train_batch_chunk_nesterov(
    ...
    forwardprop_batch(architecture, runtime, cuda_params, hidden_activation, output_activation, velocity, momentum);
    backprop_batch(architecture, runtime, batch_gradients, cuda_params, loss, target_batch, hidden_activation, output_activation, velocity, momentum);
